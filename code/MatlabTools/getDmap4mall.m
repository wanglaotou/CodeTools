...
label_name = fullfile(labelDIRS,'labelup.txt');
DmapDIRS = fullfile(mydir,'Dmap');
dmapDIRS = fullfile(DmapDIRS,'Dmap8_crowd1_5_test3');     %get Dmap4 info
% dmapDIRS = fullfile(DmapDIRS,'Dmap8');   %get Dmap8 info
% label_name = fullfile(labelDIRS,'labelall.txt');
scale = 8;  %dmap4
% scale = 8;  %dmap8
kscale = 4.0; %ksize scale
flid = fopen(label_name,'r');
while feof(flid) == 0
	line = fgetl(flid);		%'/ssd/wangmaorui/data/Label/labelroi/scene53/20171122_frame_7.txt'...
	S = regexp(line,' ','split');
	labelpath = char(S(1));
