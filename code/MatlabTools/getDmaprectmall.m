...
% dmapDIRS4 = fullfile(DmapDIRS,'Dmap4_mall');     %get Dmap4 info
dmapDIRS8 = fullfile(DmapDIRS,'Dmap8_mall');   %get Dmap8 info
mallpath = fullfile(mydir,'mall_big.txt');
% scale4 = 4;  %dmap4
scale8 = 8;  %dmap8
kscale4 = 4.0; %ksize scale
% kscale8 = 4.0;
flid = fopen(mallpath,'r');
while feof(flid) == 0
	line = fgetl(flid);		
	S = regexp(line,' ','split');
	imgpath = char(S(1));
    head = str2num(char(S(2)));
    rects = [];
    for i = 3:5:length(S)
...
%     fprintf(fdid4,'\n');
    ...
    for k=1:4:length(rects)
        ...
        rect_y = rects(k+1);
        pvalue = rects(k+2);
        pos_x = floor(rect_x + pvalue/2);
        pos_y = floor(rect_y + pvalue/2);
%         plot(pos_x,pos_y,'r*');
...
%dmap4
%         rect_x4 = floor(pos_x/scale4);
%         rect_y4 = floor(pos_y/scale4);
...
%         radius4 = (ksize4-1)/2;
% %dmap8
        rect_x8 = floor(rect_x/scale8);
        rect_y8 = floor(rect_y/scale8);
%out of boundary in the right down corner
        if(rect_x8 + pvalue/16 >= n8)
            rect_x8 = n8 - pvalue/16;
        end
        if(rect_y8 + pvalue/16 >= m8)
            rect_y8 = m8 - pvalue/16;
        end
