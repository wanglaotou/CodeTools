#show label info by random list
#Input:rectroiztall.txt
...
class Annot:
  ...
  def sparse_lines(self, line):
    ...
    for i in xrange(0, num):
      rect = [int(round(float(items[i*3+1]))), int(round(float(items[i*3+2])))]
      rects.append(rect)
    return (path, rects)
...
if __name__ == '__main__':
  ...
  for annot in annots:
    ...
    for rect in rects:
      cv2.circle(img, (rect[0],rect[1]), 2, (0,0,255),-1)
    cv2.imshow('show', img)
    cv2.waitKey(0)
